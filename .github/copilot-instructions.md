...
* **ACTION:** Synthesize all findings from the single consolidated pass (1B) into one structured report:
    ...
    6.  **Governance Audit:** Flag any governance issues found and state whether they are blocking (must be addressed now) or advisory (noted, no block). Always complete the audit step and report the result.
    7.  **Baseline Status:** List of current file errors, failed tests, and observed state.
    8.  **Clarifying Questions / Potential Pitfalls:** Any unanswered questions that could cause hallucination or incorrect implementation. **WAIT for user answers before proceeding if critical questions are raised.**
